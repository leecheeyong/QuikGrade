...
export function useAssignments(userId) {
  ...
  const fetchAssignments = async () => {
    const uid =
      userId.value && typeof userId.value === "object" && "uid" in userId.value
        ? userId.value.uid
        : userId.value;
    if (!uid) {
      console.warn(
        "No UID provided to fetchAssignments. Make sure userId is set to the authenticated user UID.",
      );
      return;
    }
